...
[Serializable]
	...
	public class MathNode : ANode
	{
		...
		public decimal Total1()
		{
			...
			//Apply formula to fields
			decimal formula_result = 0;
			EB.Math.Function function = new EB.Math.Function();
			string temp_formula = this._Formula;
			ArrayList temp_formula_ranges = new ArrayList();

			//Check to see if formula contain range expressions
			if (temp_formula.Contains("|"))
				temp_formula_ranges.AddRange(temp_formula.Split("|".ToCharArray()));


			foreach (string formula in temp_formula_ranges) { }

			//Check to see if there is a reference to a value in another node..if so find it and replace with it's total
			char[] op = new char[] { '*', '-', '/', '+' , '(' , ')' };
			string[] formula_parts = temp_formula.Split(op);
			foreach (string s in formula_parts)
			{
				if (s.Contains("\\"))
				{                   
                    try
                    {
                        if (s.EndsWith(".max")) temp_formula = temp_formula.Replace(s, this.ParentTree.GetNodeFromPath(s.Replace(".max", "")).Max.ToString());
                        else
                            if (s.EndsWith(".min")) temp_formula = temp_formula.Replace(s, this.ParentTree.GetNodeFromPath(s.Replace(".min", "")).Min.ToString());
                            else if (s.EndsWith(".discount")) temp_formula = temp_formula.Replace(s, this.ParentTree.GetNodeFromPath(s.Replace(".discount", "")).Discount.ToString());
                                else temp_formula = temp_formula.Replace(s, this.ParentTree.GetNodeFromPath(s).Total().ToString());
                    }
                    catch (CircularReferenceException)
                    {
                        throw;
                    ...
                    {
                        temp_formula = temp_formula.Replace(s, "0");
                    }
				}
                else
                    //Replace remaining values with immediate children.
                    foreach (ANode n in this.Children)
                    {
                        if (s.EndsWith(".max") && s.Replace(".max", "").Trim() == n.Name)
                        {
                            temp_formula = temp_formula.Replace(s, n.Max.ToString());
                            break;
                        }
                        else
                            if (s.EndsWith(".min") && s.Replace(".min", "").Trim() == n.Name)
                            {
                                temp_formula = temp_formula.Replace(s, n.Min.ToString());
                                break;
                            }
                            else if (s.EndsWith(".discount") && s.Replace(".discount", "").Trim() == n.Name)
                            {
                                temp_formula = temp_formula.Replace(s, n.Discount.ToString());
                                break;
                            }
                            else if (s.Trim() == n.Name)
                            {
                                temp_formula = temp_formula.Replace(n.Name, n.Total().ToString());
                                break;
                            }
                    }
			}
			...
			try
			{
				function.Parse(temp_formula);
				function.Infix2Postfix();
				function.EvaluatePostfix();
				formula_result = (decimal)function.Result;
			}
	...
	public class ConditionalNode : ANode
	{
		...
		{
			string temp_condition = this.IfCondition;
			char[] op = new char[] { '&', '|', '(', ')', '!' };
			string[] condition_parts = temp_condition.Split(op);
            string temp_s;
			foreach (string s in condition_parts)
			{
                //this is needed because the string s cannot be modified inside the loop
                temp_s = s;
				if (s.Contains(".selected"))
				{
					string node = s.Replace(".selected", "").Trim();
					if (node.Contains("\\")) //if the node is a full path
					{
						if (this.ParentTree.GetNodeFromPath(node).Selected)
							temp_condition = temp_condition.Replace(s, "1");
						else temp_condition = temp_condition.Replace(s, "0");
					}
					else //if the node is a child of current node
					{
						foreach (ANode n in this.Children)
						{
							if (node == n.Name) 
                ...
                //.discount
                if (temp_s.Contains(".discount"))
                {
                    string[] temp_s_parts = temp_s.Split(new char[] { '=', '>', '<', '*', '-', '/', '+', '(', ')' });
                    foreach (string part in temp_s_parts)
                    {
                        if (part.Trim().EndsWith(".discount"))
                        {
                            string node = part.Replace(".discount", "").Trim();
                            if (node.Contains("\\")) //if the node is a full path
                            {
                                temp_s = temp_s.Replace(node + ".discount", this.ParentTree.GetNodeFromPath(node).Discount.ToString());
                            }
                            else //if the node is a child of current node
                            {
                                foreach (ANode n in this.Children)
                                {
                                    if (node == n.Name)
                                    {
                                        temp_s = temp_s.Replace(node + ".discount", n.Discount.ToString());
                                        break;
                                    }
                                }
                            }
                ...
                //.min
                if (temp_s.Contains(".min"))
                {
                    string[] temp_s_parts = temp_s.Split(new char[] { '=', '>', '<', '*', '-', '/', '+', '(', ')' });
                    foreach (string part in temp_s_parts)
                    {
                        if (part.Trim().EndsWith(".min"))
                        {
                            string node = part.Replace(".min", "").Trim();
                            if (node.Contains("\\")) //if the node is a full path
                            {
                                temp_s = temp_s.Replace(node + ".min", this.ParentTree.GetNodeFromPath(node).Min.ToString());
                            }
                            else //if the node is a child of current node
                            {
                                foreach (ANode n in this.Children)
                                {
                                    if (node == n.Name)
                                    {
                                        temp_s = temp_s.Replace(node + ".min", n.Min.ToString());
                                        break;
                                    }
                                }
                            }
                ...
                //.max
                if (temp_s.Contains(".max"))
                {
                    string[] temp_s_parts = temp_s.Split(new char[] { '=', '>', '<', '*', '-', '/', '+', '(', ')' });
                    foreach (string part in temp_s_parts) 
                    {
                        if (part.Trim().EndsWith(".max")) 
                        {
                            string node = part.Replace(".max", "").Trim();
                            if (node.Contains("\\")) //if the node is a full path
                            {
                                temp_s = temp_s.Replace(node + ".max", this.ParentTree.GetNodeFromPath(node).Max.ToString());
                            }
                            else //if the node is a child of current node
                            {
                                foreach (ANode n in this.Children)
                                {
                                    if (node == n.Name)
                                    {
                                        temp_s = temp_s.Replace(node + ".max", n.Max.ToString());
                                        break;
                                    }
                                }
                            }
				...
				if (temp_s.Contains("="))
				{
                    string[] equality_parts = temp_s.Split("=".ToCharArray());
					if (Total(equality_parts[0].Trim()) == Total(equality_parts[1].Trim()))
                        temp_condition = temp_condition.Replace(s, "1");
                    else temp_condition = temp_condition.Replace(s, "0");
				}
                if (temp_s.Contains(">"))
			...
			return BooleanEvaluator.Evaluate(temp_condition);
		}
		...
		public decimal Total1()
		{
			...
			{
                ParentTree.TotalCounter--;
				if (this.Condition()) return this.Total(this.ThenFormula);
				else return this.Total(this.ElseFormula);
			}
            catch (CircularReferenceException)
            {
                throw;
	...
	public class ConditionalRulesNode : ANode
	{
		...
		public bool Condition(string cond) // Evaluates the condition
		{
			string temp_condition = cond;
			char[] op = new char[] { '&', '|', '(', ')', '!' };
			string[] condition_parts = temp_condition.Split(op);
            string temp_s;
			foreach (string s in condition_parts)
		...
		public decimal Total1()
		{
			...
			{
				foreach (KeyValuePair<string, string> thisRule in this.Rules)
                    if (this.Condition(thisRule.Key))
                    {
                        ParentTree.TotalCounter--;
                        return this.Total(thisRule.Value);
                    }
                ParentTree.TotalCounter--;
				return 0;
			}
	...
	public class RangeNode : ANode
	{
		...
		public decimal Total1()
		{
			...
			for (int i = 0; i < temp_formula_ranges.Length; i++)
			{
				//Replace values with immediate children.
				foreach (ANode n in this.Children)
				{
					if (temp_formula_ranges[i].Contains(n.Name)) { temp_formula_ranges[i] = temp_formula_ranges[i].Replace(n.Name, n.Total().ToString()); }
				}
				if (i > 0)
				{
					string[] values = temp_formula_ranges[i].Split(":".ToCharArray());
                    ...
                    {
                        case 3:
                            if (decimal.TryParse(values[0], out floor) && decimal.TryParse(temp_formula_ranges[0], out value) && decimal.TryParse(values[1], out ceiling))
                            {
                                if (floor <= value && value <= ceiling)
                                {
                                    decimal output;
                                    if (decimal.TryParse(values[2], out output))
                                    {
                                        ParentTree.TotalCounter--;
                                        if (this.Max != 0 && output > this.Max) return this.Amount * (Max - Max * this.Discount / 100);
                                        else
                            ...
                            break;
                        case 2:
                            if (decimal.TryParse(values[0], out floor) && decimal.TryParse(temp_formula_ranges[0], out value))
                            {
                                if (floor <= value)
                                {
                                    decimal output;
                                    if (decimal.TryParse(values[1], out output))
                                    {
                                        ParentTree.TotalCounter--;
                                        if (this.Max != 0 && output > this.Max) return this.Amount * (Max - Max * this.Discount / 100);
                                        else
